function [animals,pdfhist,Imasks,expinfo] = EB_brainimg_processing(varargin)
%% function: EB_brainimg_processing
% Description:
...
%
% OUTPUTS ---------------------------------------------------------------
% animals: structure with image information for each animal
%   FIELD           DESCRIPTION       
%   'id'            integer associated with animal identity
%   'anatomy_str'   string indicating anatomical plane ordering
%   'pad'           logical indicating whether a pad was used (1) or not (0)
%   'bld'           #animals x #anatomy matrix with logicals indicating
%   'EB'              wether the indicated pathology is visible on that
%   'hemo'            view & animal
%   'imgnames'      #animals x #anatomy cell of strings indicating the name
%                     of the tiff file in the EBfldr (see loaddir script
%                     for folder path)
...
%   FIELD     DESCRIPTION                 REQUIRES LABEL?
...
%   >> classes/fullclasses/otherclasses: cell of strings containing field
%   class names for pathologies and other classes
...
%  13 Nov  2014   Amanda Balderrama   amandagbalderrama@gmail.com     1
%     First function form using code from EB_brainimg_processing script
%     v4.3
...
expinfo.data_date = dateyyyymmddstr;
expinfo.version =  4.3;
...
pdfhist.classes = class_str;
pdfhist.otherclasses = {'bkgnd','rgb','frgb'};
pdfhist.fullclasses = full_class_str;
...
for an = 1:length(animalnumvec) % for each animal number
  ...
  if ~findmasks;  anstruct = getfield(Imasks,sprintf('an%d',ana_animalnum)); end
  ...
  save(strcat(EBfldr,'\',sprintf('%s_EBclasshistmed',dateyyyymmddstr)),...
    'pdfhist','animals','Imasks','expinfo')
  clear anstruct
end % animals for
Imasks.classes = class_str;