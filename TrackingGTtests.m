function TrackingGTtests(x0,velx,accx,y0,vely,accy,Nt,sigma_v,varargin)

%% Script: TrackingGTtests
...
%  12 Sept  2016   Amanda Balderrama  amandagbalderrama@gmail.com     0
%     Based off of HSVtrackingDebeug
...
frame            = [];  % A video frame
gtLocations = zeros(Nt+1,2);
detectedLocations = zeros(Nt+1,2);  % The detected location
trackedLocations  = zeros(Nt+1,2);  % The tracked location
kfstates = zeros(Nt+1,6);
...
% The procedure for tracking a single object is shown below.
  function trackSingleObject(param)
    ...
    for i = 0:Nt
      ...
      x = x0 + velx*i + 0.5 * accx * i^2;
      y = y0 + vely*i + 0.5 * accy * i^2;
      gtLocations(i+1,:) = [x,y];
      
      obsx = x + normrnd(0,sigma_v);
      obsy = y + normrnd(0,sigma_v);
      detectedLocations(i+1,:) = [obsx,obsy];
      
      if i == 0
        initialLocation = computeInitialLocation(param, detectedLocations(i+1,:));
        kalmanFilter = configureKalmanFilter(param.motionModel, ...
          initialLocation, param.initialEstimateError, ...
          param.motionNoise, param.measurementNoise);
      end
      
      trackedLocations(i+1,:) = correct(kalmanFilter, detectedLocations(i+1,:));
      kfstates(i+1,:) = kalmanFilter.State';
    end % while
  end
