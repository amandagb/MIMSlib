...
for iln = 1:nline
  nonnanrows = ~isnan(yp{iln}(:,1));
  dcc = bwconncomp(nonnanrows);
  for iblks = 1:dcc.NumObjects
    usei = [dcc.PixelIdxList{iblks};sort(dcc.ImageSize(1)*2 - dcc.PixelIdxList{iblks}+1)];
    hp(iln) = patch(xp{iln}(usei),yp{iln}(usei),ptchcol{iln}, 'facealpha', alpha{iln}, 'edgecolor', 'none');
  end
end
